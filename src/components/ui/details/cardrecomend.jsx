...
function CardRecomendedDetails({item}){
    return(
        <div  className="w-[150px] flex flex-col gap-1 h-full rounded-lg ">
            <a href={`/details/${item.entry.mal_id}`}  className="w-full h-[200px] rounded-lg overflow-hidden relative">
            ...
            <h1 className="text-sm text-white">{item.entry.title.length > 30 ? item.entry.title.slice(0, 30) + "..." : item.entry.title}</h1>
        </div>
    )
}
